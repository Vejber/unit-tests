...
public class Calculator {
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        ...
        if (purchaseAmount < 0 || discountAmount < 0) {
            throw new ArithmeticException("Invalid input: purchaseAmount and discountAmount must be non-negative");
        }

        double discountPercentage = ((double) discountAmount / purchaseAmount) * 100;
        double discountedAmount = purchaseAmount - discountAmount;

        // Убедимся, что сумма с учетом скидки не станет отрицательной
