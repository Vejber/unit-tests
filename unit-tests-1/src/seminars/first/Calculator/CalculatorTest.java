...
//        ).isInstanceOf(IllegalStateException.class);
        ...
        public static void testCalculatingDiscount() {
            // Проверка правильности расчета скидки
            Assertions.assertThat(Calculator.calculatingDiscount(100.0, 10)).isEqualTo(90.0);

            // Проверка для нулевой суммы покупки
            Assertions.assertThat(Calculator.calculatingDiscount(0.0, 10)).isEqualTo(0.0);

            // Проверка для нулевой скидки
            Assertions.assertThat(Calculator.calculatingDiscount(100.0, 0)).isEqualTo(100.0);

            // Проверка для максимально возможного значения скидки
            Assertions.assertThat(Calculator.calculatingDiscount(100.0, 100)).isEqualTo(0.0);

            // Проверка для недопустимых аргументов
            Assertions.assertThatThrownBy(() -> Calculator.calculatingDiscount(-50.0, 10))
                    .isInstanceOf(ArithmeticException.class)
                    .hasMessage("Invalid input: purchaseAmount and discountAmount must be non-negative");
